...
constexpr const unsigned int MAX_POSSIBLE_RESULTS =
...
constexpr auto make_span(std::array<T, N>& arr,
                         typename gsl::span<const T>::size_type count)
{
...
template <typename T, size_t N>
constexpr auto make_span(const std::array<T, N>& arr,
                         typename gsl::span<const T>::size_type count)
{
    assert((size_t)count <= arr.size());
...
inline int match_year(int val) { return val / 10000; }
inline int match_month(int val) { return (val % 10000) / 100; }
inline int match_day(int val) { return (val % 100); }
}

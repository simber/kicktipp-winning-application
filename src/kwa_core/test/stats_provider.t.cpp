#include "kwa_core/stats_provider.h"
...
std::vector<kwa::MatchData> createRandomMatches(size_t count, int max_team_id, int max_goals)
{
    ...
    std::sort(out.begin(), out.end(), [](auto lhs, auto rhs) { return lhs.day < rhs.day; });
...
TEST(StatsProvider, constr_match_count_is_zero)
{
    ...
    {
        unsigned int team_count = (static_cast<unsigned int>(std::rand()) % MAX_TEAMS) + 1;
...
{
    kwa::StatsProvider provider{};
    kwa::MatchData matches[] = {{0, 3, 2, 0, 1}, {1, 1, 2, 2, 3}, {2, 1, 2, 4, 1}, {3, 1, 3, 0, 0}};
    provider.addMatches(matches);
